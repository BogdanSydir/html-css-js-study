...
let listOfLanguages = ['html', 'css', 'javascript', 'mysql', 'mongodb', 'react', 'angular', 'node.js'];

document.write(`<ul>`)
for (let language in listOfLanguages) {
    document.write(`<li>${listOfLanguages[language]}</li>`)
}
document.write(`</ul>`)
...
for (let simpson of simpsons) {
    ...
    document.write(`PHOTO - <img class="photo" src="${simpson.photo}" alt="photo">`)
    document.write(`</div>`)
...
for (let product of products) {
    ...
    document.write(`<h3>${product.title}. Price - ${product.price}.`)
    document.write(`<img class="image" src="${product.image}" alt="image">`)
    document.write(`</div>`)
}
document.write(`</div>`)